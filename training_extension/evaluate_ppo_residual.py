...
def main():
    ...
    model = PPO.load(args.model, env=vector_env, device="auto")
    ...
    for episode in range(args.episodes):
        ...
        while not bool(done[0]):
            ...
            final_info = infos[0]
            action_deltas.append(float(final_info["rl_action_delta"]))
            ctrl_deltas.append(float(final_info["ctrl_delta"]))
            slips.append(float(final_info["foot_slip_mps"]))
        records.append(
            {
                "episode": episode,
                ...
                "mean_rl_action_delta": float(np.mean(action_deltas)),
                "mean_ctrl_delta": float(np.mean(ctrl_deltas)),
                "mean_foot_slip_mps": float(np.mean(slips)),
            }
        )
    ...
    summary = {
        ...
        "mean_crossing_error": (
            float(np.mean(crossing_errors)) if crossing_errors else None
        ),
        "max_crossing_error": (
            float(np.max(crossing_errors)) if crossing_errors else None
        ),
        ...
        "mean_rl_action_delta": float(
            np.mean([row["mean_rl_action_delta"] for row in records])
        ),
        "mean_ctrl_delta": float(
            np.mean([row["mean_ctrl_delta"] for row in records])
        ),
        "mean_foot_slip_mps": float(
            np.mean([row["mean_foot_slip_mps"] for row in records])
        ),
    }
