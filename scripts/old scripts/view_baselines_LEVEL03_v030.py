...
# ==========================================
# LEVEL 13: PERFECT HEADING & UNITED HANDS
# ==========================================
    ...
    def __init__(self, env):
        ...
        self.keyframes = {
            ...
            400: {
                ...
                'left_knee_joint': 0.4, 'right_knee_joint': 0.4, # Jump drive
            },

            # --- EXPLOSIVE SNAP (Release & Peel Away) ---
            410: { 
                # Right Arm snaps forward
                'right_shoulder_pitch_joint': -2.2, 
                'right_elbow_joint': 0.2, 
                'right_wrist_pitch_joint': 0.8, # The Gooseneck Snap
                
                # Left Arm (Guide hand) safely peels away to let the ball fly
                'left_shoulder_pitch_joint': -1.5, 'left_shoulder_roll_joint': 0.4,
...
def view_baseline():
    ...
    with mujoco.viewer.launch_passive(env.model, env.data) as viewer:
        ...
        while viewer.is_running():
            ...
            if target_body_id != -1:
                env.model.body_pos[target_body_id][0] = 1.8 # distance from the robot starting point
                env.model.body_pos[target_body_id][2] = 1.2 # height from ground
            ...
            while policy.step_count < 850 and viewer.is_running():
                ...
                # 1. Rotational Heading (Keeps robot facing perfectly forward)
                kp, kd = 100.0, 20.0   
                torque_pitch = (0.0 - pitch) * kp - (env.data.qvel[4] * kd)
                torque_roll = (0.0 - roll) * kp - (env.data.qvel[3] * kd)
                torque_yaw = (0.0 - yaw) * 50.0 - (env.data.qvel[5] * 10.0) # Corrects twisting
                
                torque_pitch = np.clip(torque_pitch, -200.0, 200.0)
                torque_roll = np.clip(torque_roll, -200.0, 200.0)
                torque_yaw = np.clip(torque_yaw, -100.0, 100.0)
                
                # 2. Lateral Translation Lock (Invisible Rails on Y-axis to prevent drifting)
                pelvis_y = env.data.qpos[1]
                pelvis_y_vel = env.data.qvel[1]
                force_y = (0.0 - pelvis_y) * 50.0 - (pelvis_y_vel * 10.0)
                force_y = np.clip(force_y, -50.0, 50.0)
                ...
                if pelvis_id != -1:
                    ...
                    # Apply lateral force (Anti-Drift)
            ...
            print(f"Max Torso Tilt (Kept extremely low by strong Gyro): Pitch {max_pitch:.2f}°, Roll {max_roll:.2f}°, Yaw {max_yaw:.2f}°\n")
