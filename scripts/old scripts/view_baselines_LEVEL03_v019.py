...
# ==========================================
# LEVEL 11: ASYMMETRICAL PRO SHOOTING FORM
# ==========================================
class OptionDBasketballPolicy:
    def __init__(self, env):
        ...
        self.keyframes = {
            # 0. STARTING STANCE (Relaxed, looking at target)
            0: { 
                ...
                'right_shoulder_pitch_joint': -0.3, 'right_elbow_joint': 0.8,
                'left_shoulder_pitch_joint': -0.3, 'left_elbow_joint': 0.8,
                'waist_pitch_joint': 0.0 
            },
            
            # --- STEP 1 & 2: RHYTHM DRIBBLE STEPS ---
            100: {
                'left_hip_pitch_joint': 0.2, 'left_knee_joint': 0.3, 'left_ankle_pitch_joint': 0.0,
                'right_hip_pitch_joint': -0.6, 'right_knee_joint': 1.0, 'right_ankle_pitch_joint': -0.3
            }, 
            
            200: {
                'right_hip_pitch_joint': 0.1, 'right_knee_joint': 0.3, 'right_ankle_pitch_joint': 0.0, 
                'left_hip_pitch_joint': -0.6, 'left_knee_joint': 1.0, 'left_ankle_pitch_joint': -0.3    
            },
            
            # --- 300: SQUARE UP TO THE BASKET (Feet set, knees slightly bent) ---
            300: {
            ...
            # --- 360: DIP / GATHER POWER (Deepening the knee bend for vertical force) ---
            360: {
                ...
                'right_shoulder_pitch_joint': -0.2, 'right_elbow_joint': 1.2, 'right_wrist_pitch_joint': -0.5,
                'left_shoulder_pitch_joint': -0.2, 'left_elbow_joint': 1.2, 'left_wrist_pitch_joint': 0.0,
            },
            
            # --- 385: THE SET POINT (Directly matches the provided photo!) ---
            385: { 
                # Shooting hand (Right): Arm parallel to ground, elbow ~90 degrees under ball, wrist deeply cocked back
                'right_shoulder_pitch_joint': -1.6, 'right_shoulder_roll_joint': 0.0,
                'right_elbow_joint': 1.8, 'right_wrist_pitch_joint': -1.2, 
                
                # Guide hand (Left): On the side of the ball, elbow slightly bent
                'left_shoulder_pitch_joint': -1.4, 'left_shoulder_roll_joint': 0.4,
                'left_elbow_joint': 1.5, 'left_wrist_pitch_joint': -0.3,
                
                # Start driving upward via legs (Kinetic chain)
                'left_hip_pitch_joint': -0.2, 'left_knee_joint': 0.4, 'left_ankle_pitch_joint': -0.2,
                'right_hip_pitch_joint': -0.2, 'right_knee_joint': 0.4, 'right_ankle_pitch_joint': -0.2,
            },

            # --- 402: UPWARD ACCELERATION ---
            402: {
                'right_shoulder_pitch_joint': -2.1, 
                'right_elbow_joint': 1.0, # Rapidly opening the elbow to transfer speed
                'right_wrist_pitch_joint': -0.5,
            },

            # --- 412: EXPLOSIVE THROW & RELEASE ---
            # NOTE: Actual mechanical ball release is perfectly timed at exactly frame 408.
            412: { 
                # Arm arcs towards the target (roughly 45 degrees elevation)
                'right_shoulder_pitch_joint': -2.4, 
                
                # LIMITING TO HUMAN LIMITS (Safe 0.4 margin guarantees no backward hyperextension)
                'right_elbow_joint': 0.4, 
                
                # THE SNAP (Wrist flick / Goose neck)
                'right_wrist_pitch_joint': 0.8, 
                
                # Guide hand falls away and stops participating
                'left_shoulder_pitch_joint': -1.6, 'left_shoulder_roll_joint': 0.6,
                'left_elbow_joint': 1.4, 
                
                # Full leg extension (Jump)
            ...
            440: {
                'right_shoulder_pitch_joint': -2.4, 
                'right_elbow_joint': 0.35, # Hold the arm straight (but safely positive)
                'right_wrist_pitch_joint': 1.2, # Hold the wrist snap perfectly
    ...
    def apply_controls(self):
        ...
        for joint_name, rad_val in targets.items():
            if joint_name in self.actuator_map:
                ...
                # HARD-ENFORCE STRICT HUMAN LIMITS ON JOINTS
                if 'elbow' in joint_name:
                    # Absolute constraint: physically guarantees no hyperextension (<0 is backwards)
    ...
    with mujoco.viewer.launch_passive(env.model, env.data) as viewer:
        ...
        while viewer.is_running():
            ...
            while policy.step_count < 850 and viewer.is_running():
                ...
                # --- GUARANTEED PERFECT RELEASE TIMING ---
