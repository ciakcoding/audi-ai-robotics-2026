...
# ==========================================
# LEVEL 03: STABLE SHUFFLE & FORWARD SHOOT
# ==========================================
class OptionDBasketballPolicy:
    def __init__(self, env):
        ...
        self.keyframes = {
            ...
            # Torso leans forward (waist_pitch) to balance the heavy arms.
            # Legs in a slight athletic crouch.
            0: { 
                ...
                'right_shoulder_pitch_joint': -2.5, 'right_elbow_joint': 1.8, 'right_wrist_pitch_joint': -0.5,
                'left_shoulder_pitch_joint': -2.2, 'left_shoulder_roll_joint': 0.5, 'left_elbow_joint': 1.5, 'left_wrist_yaw_joint': 0.5
            },
            
            # 1. STEP RIGHT (Small forward shuffle, weight shifts left to balance)
            40: { 
                'waist_pitch_joint': 0.25, 'waist_roll_joint': -0.1,
                'right_hip_pitch_joint': -0.4, 'right_knee_joint': 0.7, 'right_ankle_pitch_joint': -0.2,
            ...
            # 2. PLANT RIGHT
            60: { 
                'waist_pitch_joint': 0.25, 'waist_roll_joint': 0.0,
                'right_hip_pitch_joint': -0.4, 'right_knee_joint': 0.8, 'right_ankle_pitch_joint': -0.4,
            },
            
            # 3. STEP LEFT (Bring forward to align with right foot)
            100: { 
                'waist_pitch_joint': 0.25, 'waist_roll_joint': 0.1,
                'left_hip_pitch_joint': -0.4, 'left_knee_joint': 0.7, 'left_ankle_pitch_joint': -0.2,
            },
            
            # 4. PAUSE & STABILIZE (Feet aligned, preparing to shoot)
            120: { 
                'waist_pitch_joint': 0.20, 'waist_roll_joint': 0.0,
            ...
            # 5. THROW FORWARD! (Shoulder moves from -2.5 to -1.2, propelling ball toward hoop)
            155: { 
                'waist_pitch_joint': 0.0, # Torso snaps upright
                ...
                # The crucial fix: Swinging arm FORWARD to -1.2
                'right_shoulder_pitch_joint': -1.2, 'right_elbow_joint': 0.2, 'right_wrist_pitch_joint': 0.6,
                'left_shoulder_pitch_joint': -1.5, 'left_shoulder_roll_joint': 0.2, 'left_elbow_joint': 1.0, 'left_wrist_yaw_joint': 0.5
            }
        }
    ...
    def apply_controls(self):
        ...
        # Release the ball slightly before the arm fully extends (Frame 145)
        return t > 145 
...
def view_baseline():
    ...
    with mujoco.viewer.launch_passive(env.model, env.data) as viewer:
        ...
        while viewer.is_running():
            ...
            # Since we removed the weld in XML, we must forcibly reset the robot's base position/velocity 
            # to exactly [0,0,0.8] so it doesn't fall randomly at the start of new episodes
