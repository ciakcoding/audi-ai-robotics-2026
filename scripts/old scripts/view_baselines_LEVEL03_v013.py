    ...
    with mujoco.viewer.launch_passive(env.model, env.data) as viewer:
        ...
        while viewer.is_running():
            ...
            while policy.step_count < 500 and viewer.is_running() and not ball_released:
                ...
                torque_pitch = np.clip(torque_pitch, -60.0, 60.0)
                torque_roll = np.clip(torque_roll, -60.0, 60.0)
                ...
                for _ in range(120): 
                    for _ in range(substeps):
                        ...
                        if pelvis_id != -1:
                            ...
                            t_p = np.clip((2.0 - p) * 25.0 - (env.data.qvel[4] * 8.0), -60, 60)
                            t_r = np.clip(-r * 25.0 - (env.data.qvel[3] * 8.0), -60, 60)
