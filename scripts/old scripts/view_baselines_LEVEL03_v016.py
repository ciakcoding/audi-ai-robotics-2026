...
# ==========================================
# LEVEL 09: STANDING RECOVERY & ANALYTICS
# ==========================================
class OptionDBasketballPolicy:
    def __init__(self, env):
        ...
        self.keyframes = {
            # 0. STARTING STANCE (Feet planted firmly, flat ankles)
            0: { 
            ...
            # --- ABSORB THE RECOIL (Squat slightly to catch the balance) ---
            500: {
                'right_shoulder_pitch_joint': 0.0, 'right_elbow_joint': 0.8, 
                'left_shoulder_pitch_joint': 0.0, 'left_elbow_joint': 0.8,
                'waist_pitch_joint': 0.0,
            ...
            # --- RECOVER BALANCE (Stand tall and proud) ---
            650: {
                'right_shoulder_pitch_joint': 0.2, 'right_elbow_joint': 0.3, 
                'left_shoulder_pitch_joint': 0.2, 'left_elbow_joint': 0.3,
                'waist_pitch_joint': 0.0,
...
def view_baseline():
    ...
    with mujoco.viewer.launch_passive(env.model, env.data) as viewer:
        ...
        while viewer.is_running():
            ...
            # Raised slightly to 0.81 to give legs room, avoiding floor clipping!
            ...
            # Tracking variables for Analytics Report
            ...
            # Loop extended to 850 frames to show standing recovery!
            # ==========================================
            while policy.step_count < 850 and viewer.is_running():
                ...
                kp = 35.0  # Softened slightly to prevent jitter
                kd = 10.0   
                
                torque_pitch = (0.0 - pitch) * kp - (env.data.qvel[4] * kd)
                torque_roll = (0.0 - roll) * kp - (env.data.qvel[3] * kd)
                
                # Capped at 80.0 to prevent the feet from crushing through the floor
                torque_pitch = np.clip(torque_pitch, -80.0, 80.0)
                torque_roll = np.clip(torque_roll, -80.0, 80.0)
                ...
                if dt > 0:
                    ...
                    # 1. Predictor (Triggers Release)
                    if not ball_released and policy.step_count > 400:
                    ...
                    # 2. Analytics Tracking (After Release)
                    if ball_released:
            ...
            print(f"if the ball has crossed the basketball hoop: {'YES!' if ball_crossed_hoop else 'NO'}")
